...
# import_shard0.bat/import_shard1.bat files, or a scheduled task), so you
# can reclaim the GPU/Ollama for other work. Safe to run any time:
# LangGraph checkpoints after every completed graph node, so at most the
# in-flight item's call is lost — the next run picks back up from there via
# --continue. See README.md.
