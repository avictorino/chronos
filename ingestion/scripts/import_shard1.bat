@echo off
REM Launched daily at 01:00 by the "ChronosImportShard1" Windows Scheduled
REM Task (see ingestion/scripts/README.md). Covers civilizations at odd
REM indexes of data/civilizations.yaml (index 1, 3, 5, ...) — includes
REM Babylon. Runs at the same time as import_shard0.bat (even indexes,
REM includes Assyria), each in its own process with its own checkpoint DB,
